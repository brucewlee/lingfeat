...
def retrieve(origin_doc, token_list, n_token, n_sent):
    ...
    result = {
        "as_Token_C":float(division(total_count_tokn,n_sent)),
        "as_Sylla_C":float(division(total_count_syll,n_sent)),  
        "at_Sylla_C":float(division(total_count_syll,n_token)),       
        "as_Chara_C":float(division(total_count_char,n_sent)),   
        "at_Chara_C":float(division(total_count_char,n_token)),
    }
    return result